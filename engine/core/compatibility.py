"""GameHunter Engine - Game Compatibility Checker.

This module analyzes game system requirements against user hardware to determine
whether a game can run and predict performance levels.

The compatibility checking process:
1. Parse unstructured requirement text into normalized key-value pairs
2. Extract numeric values (RAM, disk space) from requirements
3. Score user hardware using the hardware_engine module
4. Compare user specs against minimum and recommended requirements
5. Generate detailed compatibility report with pass/fail status

Key Components:
    _KEY_MAP: Maps common requirement label variations to normalized keys
    extract_requirements: Parses requirement text into structured data
    parse_requirement_value: Extracts numeric values from text
    check_compatibility: Main compatibility analysis function
"""
...
# Requirement key normalization mapping
# Maps various label formats to standardized keys
_KEY_MAP = {
    "OS": ["OS", "Windows"],
...
def extract_requirements(text: str) -> Dict[str, str]:
    """Parse unstructured game requirement text into normalized key-value pairs.

    Extracts requirement categories from free-form text and normalizes labels.

    Args:
        text: Raw requirement text from the game API.

    Returns:
        dict: Normalized requirements with standard keys.
    """
    if not text:
        return {}
    specs = {}
...
def parse_requirement_value(text: str, pattern: str, default: float = 0) -> float:
    """Extract numeric value from requirement text using regex.

    Parses values like 8 GB RAM into normalized GB values.
    ...
    Returns:
        float: Extracted value in GB, or default if not found.
    """
    if not text:
        return default
...
def check_compatibility(
    user_specs: Dict[str, Any], requirements: Dict[str, Optional[str]]
) -> Dict[str, Any]:
    """Check if user hardware can run a game.

    Performs comprehensive compatibility analysis by comparing user specs against
    game requirements. Analyzes RAM, disk, CPU, and GPU, then predicts graphics preset.

    Args:
        user_specs: User system specifications with keys: cpu, ram, gpu, disk_free.
        requirements: Game requirements with minimum and/or recommended strings.

    Returns:
        dict: Compatibility report with overall status, detailed analysis per component,
              warnings, and predicted graphics preset.
    """
    ...
    if u_ram < min_ram:
        ...
        if u_ram >= rec_ram * 1.5:
            ...
            ram_status = "Passed"
            ram_msg = (
                "Your RAM meets the recommended specifications for a smooth experience."
            )
        elif u_ram >= min_ram:
    ...
    # --- Disk ---
    disk_pat = (
        r"(\d+(?:\.\d+)?)\s*(GB|MB)\s*(?:available|free|storage|space|hard drive)"
    )
    min_disk = parse_requirement_value(
        min_p.get("Storage", "") or min_req_str, disk_pat
    )

    disk_status = "Passed"
    disk_msg = (
        "Please ensure you have enough space on your preferred installation drive."
    )
    ...
    def get_comp_status(u_score, m_score, r_score, comp_type="GPU"):
        ...
        if u_score >= target * 1.5:
            return (
                "Passed",
                f"Your {comp_type} is a powerhouse! It's more than enough for this game.",
            )
        ...
        if u_score >= m_score * 1.1:
            if r_score > 0 and u_score < r_score * 0.9:
                return (
                    "Minimum Met",
                    "Meets minimum requirements, but below recommended performance.",
                )
        ...
        if u_score >= m_score * 0.9:
            return (
                "Minimum Met",
                "Meets minimum requirements, but performance may be marginal.",
            )
        ...
        if u_score < m_score * 0.7:
            return (
                "Failed",
                f"Your {comp_type} is significantly below the required power level.",
            )

        return (
            "Check Manually",
            "Performance is on the border; may struggle at low settings.",
        )
    ...
    if (
        "UHD" in u_gpu.upper()
        or "IRIS" in u_gpu.upper()
        or "INTEGRATED" in u_gpu.upper()
    ):
...
def get_compatibility_checker():
    """Factory function to get the compatibility checker.

    Returns:
        function: The check_compatibility function.
    """
